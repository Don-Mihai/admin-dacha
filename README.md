...
Появится файл `utkina-admin.exe`. На Windows поместите exe в папку `admin` внутри корня проектов (рядом с папками проектов). Запуск exe откроет сервер, откройте в браузере **http://localhost:3333**.
